...
const SYSTEM_PROMPT = `You are a form schema generator. You analyze images of forms and produce a JSON schema that recreates them.
...
Now analyze the provided form image and generate a complete JSON schema.`;
...
export async function POST(req: NextRequest) {
  ...
  // Accept JSON body: { images: string[] }
  ...
  try {
    ...
    if (!Array.isArray(images) || images.length === 0) {
      return NextResponse.json(
        { error: "No images provided. Expected { images: string[] }" },
        { status: 400 }
      );
    }
  } catch {
    return NextResponse.json(
      { error: "Invalid JSON body. Expected { images: string[] }" },
      { status: 400 }
    );
  }
  ...
  const userContent: OpenAI.Chat.Completions.ChatCompletionContentPart[] = [
    {
      ...
      text:
        images.length > 1
          ? `Analyze these ${images.length} form page images and generate a single unified JSON schema that recreates the entire form. Include appropriate field types, labels, placeholders, options, validation, and any logical client field linkings. Use layout elements (headers, dividers, content blocks) to structure the form. Pair related fields as half-width where it makes sense.`
          : "Analyze this form image and generate a JSON schema that recreates it. Include appropriate field types, labels, placeholders, options, validation, and any logical client field linkings. Use layout elements (headers, dividers, content blocks) to structure the form. Pair related fields as half-width where it makes sense.",
    },
    ...images.map(
